...
A π automaton is a 5-tuple _A = (G, Q, δ, q0, F)_, where G is a context-free grammar, Q is the set of states, q0 is the initial state, F ⊆ Q is the set of final states and
_δ : L(G)* × L(G)* × Store → Q_, where _L(G)_ is the language generated by _G_ and _Store_ represents the memory. (Elements in a set _S*_ are represented by terms _[s₁, s₂, ..., sn]_.)
...
<Exp>       ::= <ArithExp> | <BoolExp> | Id(<String>) 
...
Recall that _δ : L(G)* × L(G)* × Store → Q_, and let _N, Nᵢ ∈ BbbN    _ , _B, Bᵢ ∈ Bool_, _C, V ∈ L(G)*_, _S ∈ Store_,
...
* Notation _h :: ls_ denotes the concateBbbNion of element _h_ with the list _ls_. 
...
That's why _Nᵢ_ are BbbNural numbers and _Bᵢ_ are Boolean values.
...
* _Store = Id ↦ Storable_, _Env = Loc ↦ Bindable_, _Loc ⊆ Store_, _BbbN ⊆ Loc_, _BbbN ⊆ Bindable_.
...
* In π, a bindable value is either a Boolean value, an integer or a location. 
...
Let _F ∈ Formals_, _B ∈ Blk_, _I ∈ Id_, _A ∈ Actuals_, _Vᵢ ∈ Value_, _1 ≤ i ≤ n_, _n ∈ BbbN_,  
