...
Recall that _δ : L(G)* × L(G)* × Store → Q_, and let _N, Nᵢ ∈ Nat_ , _B, Bᵢ ∈ Bool_, _C, V ∈ L(G)*_, _S ∈ Store_,
...
* _Store = Id ↦ Storable_, _Env = Loc ↦ Bindable_, _Loc ⊆ Store_, _Nat ⊆ Loc_, _Nat ⊆ Bindable_.
...
Let _F ∈ Formals_, _B ∈ Blk_, _I ∈ Id_, _A ∈ Actuals_, _Vᵢ ∈ Value_, _1 ≤ i ≤ n_, _n ∈ Nat_,  
