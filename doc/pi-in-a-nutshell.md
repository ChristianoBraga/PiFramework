...
_𝛅 : L(G)* × L(G)* × Store → Q_, where _L(G)_ is the language generated by _G_ and _Store_ represents the memory. 
...
Recall that _𝛅 : L(G)* × L(G)* × Store → Q_, and let _N, Nᵢ ∈ ℕ_ , _B, Bᵢ ∈ Bool_, _C, V ∈ L(G)*_, _S ∈ Store_,
...
* The transition function is _𝛅 : L(G)* × L(G)* × Env × Store → Q_, and let _W ∈ String_,  _C, V ∈ L(G)*_, _S ∈ Store_, _E ∈ Env_, _B ∈ Bindable_, _l ∈ Loc_, _T ∈ Storable_, _X ∈ `<Exp>`_, _M, M₁, M₂ ∈ `<Cmd>`_,and expression _S' = S/[l ↦ N]_ means that _S'_ equals to _S_ in all indices but _I_ that is bound to _N_,
...
Let _BlockLocs_ = 𝒫(Loc), the transition function be _𝛅 : L(G)* × L(G)* × Env × Store × BlockLocs → Q_, and
